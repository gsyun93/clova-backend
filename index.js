...
app.post('/generate-helper-person', async (req, res) => {
  try {
    ...
    const helperPersonResult = parseHelperPersonResult(text);
    console.log('=== 파싱된 조력자 결과 ===');
    console.log(helperPersonResult);
    console.log('=== 파싱 결과 끝 ===');
    
    res.json(helperPersonResult);
...
app.post('/generate-hindrance-person', async (req, res) => {
  try {
    ...
    const hindrancePersonResult = parseHindrancePersonResult(text);
    console.log('=== 파싱된 방해꾼 결과 ===');
    console.log(hindrancePersonResult);
    console.log('=== 파싱 결과 끝 ===');
    
    res.json(hindrancePersonResult);
...
조건:
- 연애와 휴식은 각각 20% 이상이 자주 나오도록 강조하세요
...
[신년 운세 요약 생성 요청]
- 출력 형식: **딱 1문장, 40~50자 사이의 자연스러운 한 문장**
- 반드시 포함: 태어난시간의 지지(80%), 별자리(10%), 띠(10%) 중 하나를 포함하여 문장 생성
- 형식 권장: "초반에는... 후반에는..." 구조로 2026년 전체의 흐름을 표현
- 내용 권장: 재물운, 연애운, 직업운, 건강운 중 2~3개를 자연스럽게 암시적으로 언급
...
예시)
- 자시에 태어난 너는 2026년 초반 직업운에서 중요한 기회를 잡고, 후반에는 재물운과 건강운이 함께 상승할 거야
- 천칭자리는 2026년 초반 연애운에 뜻밖의 만남이 생기고, 후반에는 그 만남이 재물운과 직업운으로 이어질 거야
- 말띠는 2026년 초반 재물운이 안정적으로 흐르다가, 후반에는 연애운과 직업운이 크게 상승해 건강운까지 좋아질 거야

${data.mbti ? `[MBTI별 신년 가이드 요청]
- 출력 형식: **딱 1문장, 40~50자 사이의 자연스러운 한 문장**
- 반드시 포함: MBTI 유형 + 실행 유도 행동 제안
- 형식 권장: "초반에는... 후반에는..." 구조로 2026년 전체의 흐름을 표현
- 내용 권장: 재물운, 연애운, 직업운, 건강운과 연계된 구체적인 행동 제안
...
예시)
- ENFP는 신년 초반 연애운이 좋아지는 시점에 새로운 사람들과 만나야 하고, 후반에는 그 만남들이 직업운과 재물운으로 이어질 거야
- INTJ는 신년 초반 직업운에서 중요한 결정을 내려야 하는데, 그 결정이 후반에는 재물운과 건강운 상승으로 이어질 거야
- ISFP는 신년 초반 건강운을 챙기면서 새로운 취미를 시작하면, 후반에는 그 취미가 직업운과 재물운으로 발전할 거야` : ''}
...
function parseRegretPreventionResult(text) {
  ...
  try {
    ...
    return {
      ...
      regret_interpretation: extractTextBlock(lines, 'regret_interpretation:') || '오늘은 충동적으로 중요한 결정을 내릴 위험이 있어, 한 번 더 생각해야 합니다.',
...
function parseHelperPersonResult(text) {
  ...
  try {
    ...
    return {
      helper_person_title: extractTextBlock(lines, 'helper_person_title:') || '따뜻한 조언자',
      helper_person_interpretation: extractTextBlock(lines, 'helper_person_interpretation:') || '오늘은 주변에서 조언을 해줄 사람이 나타날 거야, 그 말에 귀 기울여봐',
      helper_person_message: extractTextBlock(lines, 'helper_person_message:') || '낯선 사람의 제안도 한 번 들어보면 예상치 못한 도움이 될 거야'
    };
  }
}
